...
workflow_failure_trigger = TriggerType(
    name='workflow_failure',
    description='Triggering that occurs on the failure of a specific workflow',
    params_json_schema={
        'type': 'object',
        'properties': {
            'failed_workflow_id': {
                'type': 'string',
                'description': 'Trigger fires whenever this workflow fails'
            },
        },
        'additionalProperties': False,
        'required': ['failed_workflow_id'],
    }
)


@injectable
class WorkflowFailureTriggerProcessor(TriggerProcessor):
    def __init__(self, trigger_proxy, workflow_service):
        self._trigger_proxy = trigger_proxy
        self._workflow_service = workflow_service
        self._trigger_type = workflow_failure_trigger
    ...
    def evaluate_message(self, message, trigger_service):
        ...
        executed_trigger_ids = []
        arg = {'failed_workflow_id': message['workflow_id']}
        for trigger in trigger_service.find_triggers(self._trigger_type.name, arg):
            execute_trigger(trigger, self._trigger_type, self._workflow_service, _logger)
            executed_trigger_ids.append(trigger.id)
    ...
    def send_evaluation_message(self, workflow_id):
        self._trigger_proxy.trigger_workflow_failure(workflow_id)

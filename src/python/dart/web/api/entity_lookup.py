...
def check_login(f):
    ...
    def wrapper(*args, **kwargs):
        func = f
        # if we have a session["user_id"] than we are logged it (it is removed on logout)
        if not session.get('user_id'):
          if not request.headers.get('Authorization'): # Authorization header means a browser-less api mode.
            func = login_required(f)
          else:
            pass #TODO - add api key
